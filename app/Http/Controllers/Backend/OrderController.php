...
class OrderController extends Controller
{
    ...
    {
        return view('backend.orders.invoice');
    } // end mehtod
    ...
    {
        $order = Order::with('district', 'postCodes', 'user')->where('id', $order_id)->first();
        //dd($order);
        $orderItem = OrderItem::with('product')->where('order_id', $order_id)->orderBy('id', 'DESC')->get();
    ...
    public function DeliveryCode($role, $id, $print_quantity)
    {
        $order = Order::with('orderItems.product','district','postCodes','user')->where('id', $id)->first();
        //dd($order);
        return view('backend.pdf.delivery_code', compact('print_quantity', 'id', 'order'));
    }
    
    // pending orders details
     public function InvoicePrintBpp($role,$order_id){
     
        $order = Order::with('division','district','state','user','postCodes')->where('id',$order_id)->first();
		$orderItem = OrderItem::with('product')->where('order_id',$order_id)->orderBy('id','DESC')->get();
		$bppshops = SiteSetting::all();
    	return view('backend.orders.invoice', compact('orderItem', 'bppshops', 'order'));
     }// end mathod
