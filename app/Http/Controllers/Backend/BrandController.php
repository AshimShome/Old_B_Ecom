...
class BrandController extends Controller
{
    ...
    public function searchBrandByAjax($adminId, $value){
        $admin = Admin::where('id',$adminId)->first();
        $employeePermision = json_decode($admin->permission,true);
        $employee=$admin->employee_id;

        $brands = Brand::join('ecommerce__names','brands.ecom_id','ecommerce__names.id')
                        ->where('brand_name_cats_eye', 'LIKE', "$value%")
                        ->get();
        return response()->json(['brands'=>$brands,'employeePermision'=>$employeePermision,'employee'=>$employee]) ;
